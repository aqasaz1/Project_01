  ...
  $.ajax({
      ...
      success: function onData (data) {
        ...
        if($("."+arry_cod).length < 1) {
          ...
          var content_seq = '<table class="main_contable '+set_cod_step +' table2excel_with_colors" style="display:blank">' //style="display:none"
...
function _content_xls_download(file_name){
  return 
  var table = $('.'+file_name);
  var preserveColors = (table.hasClass('table2excel_with_colors') ? true : false);
  $(table).table2excel({
...
function xls_set_hunet_index(item){
  ...
  var content_seq = '<table class="hunet '+file_name +' table2excel_with_colors" >' //style="display:none"
  content_seq += '	<thead>'
  content_seq += '		<tr>'
